        ...
        if(typeof(cep) !== 'undefined'){
            if(cep.trim().length === 8){
                ...
                if(localStorage.getItem(`${cep}`)){
                    ...
                    $.post('/api',ajaxParams,(apiReturnData)=>{
                        try {
                            let viaData = JSON.stringify(apiReturnData.resul);
                            if(viaData === '"success"'){
                                localStorage.setItem(`${cep}`,`${JSON.stringify(apiReturnData.message)}`);
                                cepTable(apiReturnData.message);
                            }else{
                                message('danger',apiReturnData.message,'buscarInfos');
